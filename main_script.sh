...
setup_repository() {
    if [ -d "$REPO_DIR" ]; then
        ...
        read -p "Репозиторий уже существует. Обновить его? Не рекомендую. (y/n): " choice
...
select_strategy() {
    ...
    echo "Доступные дурения (на данный момент 11 рабочая):"
    select strategy in "${bat_files[@]}"; do
        if [ -n "$strategy" ]; then
            log "Выбран протокол дурения: $strategy"
            cd ..
            break
        fi
...
setup_nftables() {
    ...
    log "запуск фентанилового реактора..."
...
start_nfqws() {
    log "Подключение к серверам Моссада..."
    sudo pkill -f nfqws
    cd "$REPO_DIR" || handle_error "Не удалось перейти в директорию $REPO_DIR"
...
main() {
    ...
    if $NOINTERACTIVE; then
        ...
        echo "Доступные сетевые интерфейсы (использовать enp3s0):"
